...
class DataController:
    ...
    # inserts new record in table rt_config with updated lut for channel_id and versions_id, prompt for version_desc
    #     def store_lut( self, channel):
    #         '''Stores a new record into rt_config table with new LUT in two steps:
    ...
    def estimate_vb(self, v, channel):
        '''Given measured voltage, v, and appropriate LUT, Estimate the battery voltage, vb.'''
        alut = self.luts[channel]
        ord_keys = sorted(alut.keys())
        limits = self.lut_limits[channel]
        if limits.lower_limit > v or limits.upper_limit < v:
